...
/* Define to have event handler absorb interrupts during select */
/*#define CONTINUE_ON_INTR */
...
struct event_data{
    ...
    union {
	int (*timeout)(void *);		/* callback for timer events */
	int (*fd)(int, void *);		/* callback for file events */
    } e_fn;                             /* callback function */
		   ...
		   char *str)
{
    ...
    e->e_arg = arg;
    e->e_type = EVENT_TIME;
    t0.tv_sec = usecs / 1000000; t0.tv_usec = usecs % 1000000 * 1000;
...
static int
event_delete(struct event_data **firstp, int (*fn)(void*), 
		  void *arg)
{
	...
	if (fn == e->e_fn.timeout && arg == e->e_arg) {
	    *e_prev = e->e_next;
	    free(e);
	    return 0;
	}
...
event_timeout_delete(int (*fn)(void*), 
		  void *arg)
{
    return event_delete(&ee_timers, (int (*)(void*)) fn, arg);
}
...
event_fd_delete(int (*fn)(int, void*), 
		  void *arg)
{
    return event_delete(&ee, (int (*)(void*)) fn, arg);
}
...
eventloop()
{
...
#ifdef CONTINUE_ON_INTR
		continue;
#else		
		return -1;
#endif /* CONTINUE_ON_INTR */
	    }
	}
	else if (n == 0) {  /* Timeout */
	    e = ee_timers;
	...
	else { /* Data available on some files */
	    e = ee;
	    while (e) {
		e1 = e->e_next;
